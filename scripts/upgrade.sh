...
if [ -f $FILE ]; then
    ...
    RAW_GITHUB_URL=https://raw.githubusercontent.com/chainbase-labs/easegress-providerproxy
...
if [[ "${OS}" == "linux" ]]; then
    ...
    systemctl -q is-active easegress.service && echo "Stop the easegress service" && systemctl stop easegress.service


    echo "Start the easegress service"
    systemctl start easegress.service
fi
